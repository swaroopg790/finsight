...
// Distinct palette — readable on light background
const COLORS = [
...
export default function AllocationChart({ hasHoldings }: Props) {
  ...
  // Cap at 8 slices — lump the tail as "Other"
  const MAX_SLICES = 8
  let chartData = data
  if (data.length > MAX_SLICES) {
    ...
    const otherWt    = rest.reduce((s, d) => s + d.weightPct, 0)
    chartData = [...top, { ticker: 'OTHER', name: 'Other', value: otherValue, weightPct: otherWt }]
  }
